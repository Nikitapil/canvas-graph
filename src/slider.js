import {circle, computeBoundaries, computeXRatio, computeYRatio, css, isOver, line, noop, toCoords} from "./utils";
...
export function sliderChart(root, data, DPI_WIDTH) {
    ...
    function draw(internalData) {
        ctx.clearRect(0, 0, DPI_WIDTH, DPI_HEIGHT)
        internalData.map(toCoords(xRatio, yRatio, DPI_HEIGHT, -5, yMin)).forEach((coords, idx) => {
            const color = data.colors[internalData[idx][0]]
